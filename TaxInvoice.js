	...
	setTimeout(function()
		{
			...
			$('#invoice tr').each(function (index, row){
				if(index > 0)
				{
						...
						rateValue = TaxInvoice.deductPercentage( percentageValue, mrpValue );
						rateValue = TaxInvoice.deductVAT(rateValue);
...
var TaxInvoice = 
{
	...
	populateRowData: function(selectedTextBox)
	{
		...
		$('td:eq(1), td:eq(4)', selectedRow).each(function(index) {
			...
			if(index == 1)
			{
				...
				var rate = TaxInvoice.deductPercentage( percentage, mrp );
				rate = TaxInvoice.deductVAT(rate);
				$(this).text(rate);
			}
		});
	},
	...
	deductPercentage: function(percentage, mrp)
	{
		valueToDeduct = (mrp*percentage) /100;
		return mrp - valueToDeduct;
	},
	
	deductVAT: function(amount)
	{
		var VATAmount = 1.135;
		valueToDeduct = (amount/VATAmount);
